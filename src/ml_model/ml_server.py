#!/usr/bin/env python3
"""
Persistent ML predictor server that loads the model once and serves predictions via stdin/stdout
This avoids the overhead of spawning a new Python process for each prediction
"""
...
def predict_importance_scores(model, num_vars, features):
    ...
    # The model predicts optimal sparsity, we convert this to importance scores
    feature_vec = construct_feature_vector(features)
    
    try:
        # Predict sparsity (single value)
        sparsity = model.predict(feature_vec.reshape(1, -1))[0]
        sparsity = max(0.15, min(0.85, float(sparsity)))
        
        # Convert to per-variable importance scores based on occurrences
        var_occurrences = features[4:] if len(features) > 4 else []
        
        if var_occurrences and len(var_occurrences) >= num_vars:
            # Normalize occurrences to importance scores
            ...
            if max_occ > 0:
                ...
                # More frequent variables get higher importance
        ...
        print(f"ERROR: Prediction failed: {e}", file=sys.stderr, flush=True)
